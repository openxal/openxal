    ...
    */
        ...
        throws ModelException
    {
    	...
    	if (N==0 && getOrientation() == IElectromagnet.ORIENT_HOR)
    	{
    		...
    		matPhi.setElem(0,5,gamma*gamma*h*(1-Math.cos(kx*Deltas))/Math.pow(kx,2));
    		matPhi.setElem(1,5,gamma*gamma*h*Math.sin(kx*Deltas)/kx);
    	}
    	...
    	if (N>0 && N<1 && getOrientation()==IElectromagnet.ORIENT_HOR)
    	{
    		...
    		matPhi.setElem(4,2,-h*Math.sin(kx*Deltas)/kx);//OK
    		matPhi.setElem(4,3,-h*(1-Math.cos(kx*Deltas))/Math.pow(kx,2));

    		matPhi.setElem(2,5,gamma*gamma*h*(1-Math.cos(kx*Deltas))/Math.pow(kx,2));
    		matPhi.setElem(3,5,gamma*gamma*h*Math.sin(kx*Deltas)/kx);
       	}
