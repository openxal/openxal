#==================================================================
# This script to generate Delta-T input files for Scan1D analysis
#                           CCL3
#==================================================================
import sys
import math
...
# method to get time extra time from 1/2 a BPM stripline length
def Half_BPM_Time(state,length_bpm):
	gamma = 1. + state.getKineticEnergy()/state.getSpeciesRestEnergy()
	beta = math.sqrt(1. - 1./(gamma*gamma))
	v = IConstants.LightSpeed * beta
	return ((length_bpm/2.0)/v)
...
def setBPM_Phases(traj,nodeT0,bpms,ph_s):
	...
	for i in xrange(len(bpms)):
		bpm = bpms[i]
		length_bpm = bpm.getBucket("bpm").getAttr("length").getDouble()
		orientation = bpm.getBucket("bpm").getAttr("orientation").getInteger()
		#length_bpm = 0.0
		#orientation = 0
		state = traj.statesForElement(bpm.getId())[0]
		t = state.getTime() - t0 - orientation*Half_BPM_Time(state,length_bpm)
		ph_s[i] = getPhase(t)
...
bpms_nodes_to_use = [ "SCL_Diag:BPM00b" ,\
		              "SCL_Diag:BPM01" ,\
		              "SCL_Diag:BPM03" ,\
		              "SCL_Diag:BPM04" ,\
		              "SCL_Diag:BPM05"]
...
file_names = ["ccl_4_bpm00b_bpm03.dat",\
	          "ccl_4_bpm00b_bpm04.dat",\
	          "ccl_4_bpm01_bpm04.dat",\
	          "ccl_4_bpm01_bpm05.dat"]
#+++++++++++++++++++++++++++++++++++++++++++++
...
for bpm in bpms:
	length_bpm = bpm.getBucket("bpm").getAttr("length").getDouble()
	orientation = bpm.getBucket("bpm").getAttr("orientation").getInteger()
	print "==BPM node=",bpm.getId()	,"    length=",length_bpm," orientation =",orientation
...
print "Nominal values: W0 (eV) = ", WIn0
print "Design phase (deg)= " , phase0
phase0 = phase0 - 0.;
print "New design phase (deg) = " , phase0
print "Design Amp (MV/m) = ", amp0
...
#-----------------------------------------------------------------
theProbe.reset()
theProbe.setKineticEnergy(WIn0)
theCav.updateDesignPhase(phase0)
theModel.resync()
theModel.run()
...
for i_pair in xrange(len(bpms_pairs)):
	...
	for i_amp in xrange(len(ampFactors)):
		...
		if(-a_matr[1][1] >= 0.):
			...
			Angle_arr[i_pair][i_amp] = math.atan(-a_matr[1][0]/a_matr[1][1]) * (180./math.pi) - 180.
