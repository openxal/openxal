...
class JcaSystem extends ChannelSystem {
    ...
    public JcaSystem( final String contextName ) {
		try {
			...
			if("com.cosylab.epics.caj.CAJContext".equals(contextType)) {
				String jcaLogging = System.getProperty("jcaLoggingLevel", "WARNING").toUpperCase();
				Logger logger = Logger.getLogger("com.cosylab.epics.caj.CAJContext");
				switch (jcaLogging) {
					case "INFO":
