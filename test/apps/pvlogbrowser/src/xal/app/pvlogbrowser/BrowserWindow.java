...
import xal.app.pvlogbrowser.apputils.*;
 ...
 * @author tap
 */
public class BrowserWindow extends XalWindow implements SwingConstants, ScrollPaneConstants {
	/** browser model */
	...
	/** Table model for displaying channels of the selected group */
	protected ChannelTableModel _channelTableModel;
	 ...
	 */
	public BrowserWindow(BrowserDocument aDocument, BrowserModel model) {
		super( aDocument );
	...
	protected Container buildQueryView() {
		...
		_groupMenu.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent event) {
				try {
					_model.selectGroup( (String)_groupMenu.getSelectedItem() );
				}
				catch( Exception exception ) {
	...
	protected Container buildSignalView() {
		...
		PatternEventPoster patternFilter = new PatternEventPoster( signalFilterField.getDocument(), false );
		...
		selectButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				_channelTableModel.setFilteredSignalsSelected(true);
			}
		});
		...
		unselectButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				_channelTableModel.setFilteredSignalsSelected(false);
			}
		});

		_channelTableModel = new ChannelTableModel(_controller);
		_channelTableModel.setPatternFilter(patternFilter);
		JTable channelTable = new JTable(_channelTableModel);
		signalView.add(channelTable.getTableHeader());
		signalView.add(new JScrollPane(channelTable));
		TableColumn selectionColumn = channelTable.getColumnModel().getColumn( ChannelTableModel.SELECTION_COLUMN );
	...
	protected Container buildSnapshotListView() {
		...
		final MachineSnapshotTableModel machineSnapshotTableModel = new MachineSnapshotTableModel();
		_model.addBrowserModelListener( machineSnapshotTableModel );
		...
		snapshotTable.getSelectionModel().addListSelectionListener(
				new ListSelectionListener() {
					public void valueChanged( ListSelectionEvent event ) {
						if (!event.getValueIsAdjusting()) {
							...
							else if ( selectedRow < machineSnapshotTableModel.getRowCount() ) {
								MachineSnapshot selectedSnapshot = machineSnapshotTableModel.getMachineSnapshotAt( selectedRow );
	...
	protected Container buildSnapshotDetailView() {
		...
		final SnapshotDetailTableModel detailTableModel = new SnapshotDetailTableModel( _controller );
		_controller.addBrowserControllerListener( detailTableModel );
		...
		final ChannelSnapshotValueTableModel valueTableModel = new ChannelSnapshotValueTableModel();
		...
		snapshotDetailTable.getSelectionModel().addListSelectionListener( new ListSelectionListener() {
			public void valueChanged( final ListSelectionEvent event ) {
				if ( !event.getValueIsAdjusting() ) {
					...
					valueTableModel.setChannelSnapshot( selectedRow >= 0 ? detailTableModel.getChannelSnapshot( selectedRow ) : null );
				}
			}
		});
	...
	protected void updateGroupMenu() {
		try {
			...
			final String[] types = _model.getLoggerTypes();
			
			if ( types == null ) {
				_model.selectGroup( null );
				return;
			}
			...
			if ( types.length > 0 ) {
				_model.selectGroup( typeList.get( 0 ) );
			}
		}
		catch( Exception exception ) {
