...
public class UpdatingController {
    ...
    private final Vector<ArrayDataPV> dataPVs = new Vector<ArrayDataPV>();

    private volatile boolean updateInProgress = false;

    private volatile boolean threadInProgress = false;

    private final Vector<ActionListener> listenersV = new Vector<ActionListener>();
    ...
    public void setSyncObj(Object syncObjNew) {
        synchronized (syncObj) {
            synchronized (syncObjNew) {
                if (dataPVs.size() != 0) {
                    Vector<UpdatingController> cntrlsAll = new Vector<UpdatingController>();
                    for (int i = 0; i < dataPVs.size(); i++) {
                        ArrayDataPV ard = dataPVs.get(i);
                        Vector<UpdatingController> cntrls = ard.getControllers();
                    ...
                    for (int j = 0; j < cntrlsAll.size(); j++) {
                        (cntrlsAll.get(j)).setSyncObjSimple(syncObjNew);
                    }
                }
