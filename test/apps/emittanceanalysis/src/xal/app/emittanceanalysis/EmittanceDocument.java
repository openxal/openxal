...
public class EmittanceDocument extends XalDocument {
	...
	DoubleInputTextField  distanceField = new DoubleInputTextField(9);
	...
	public void readEmittanceAnalysisDocument(URL url) {
		...
		if (readAdp != null) {
			...
			if (emittData_Adaptor != null) {
				...
				if(emittData_Adaptor.hasAttribute("harp_slit_distance")){
					double distance = emittData_Adaptor.doubleValue("harp_slit_distance");
					distanceField.setValue(distance);					
				}
				else {
					distanceField.setValue(0.0);
				}
			...
			//	rawDataPanel.setDataFromXML(emittData_Adaptor);   //we will not read wire data, because document is being used for parameters only
			}
		}
	}
	...
	public void saveDocumentAs(final URL url) {
		...
		emittData_Adaptor.setValue("harp_slit_distance", distanceField.getValue());
	...
	private void makePreferencesPanel() {
		...
		JPanel distancePanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 1, 1));
		
		distancePanel.setBorder(BorderFactory.createTitledBorder(etchedBorder, "Set Slit-Harp (Slit) Distance [cm]"));
		...
		tmpPanel.add(distancePanel,BorderLayout.CENTER);
		...
		distanceField.setValue(0.0);
		final XalDocument doc = this;
		distanceField.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				rawDataPanel.setDistance(distanceField.getValue());
				doc.setHasChanges(true);
			}
