...
public final class RawDataPanel {
	...
	public RawDataPanel() {
	    ...
	    private static final int NUMBERWIDTH = 12;
    
    private DecimalFormat dataFormat = null;
    ...
    public boolean saveEmittanceData() {

        EmittanceRawData data = makeRawToEmittancePanel.getEmittanceRawData();
        ...
        if (data.isInitialized()) {
            ...
            if (rawFile != null) {
                ...
                dataFile = new File(path.substring(0, path.lastIndexOf(".")) + ".txt");
            ...
            try {
                ...
                int nx = makeRawToEmittancePanel.getResolutions()[2];
                int nxp = makeRawToEmittancePanel.getResolutions()[3];

                double xpMax = data.getMaxAngl();
                double xpMin = data.getMinAngl();
                double xMin = data.getMinPos();
                double xMax = data.getMaxPos();

                double step = (xMax - xMin) / (double) nx;
                double x = xMin+makeRawToEmittancePanel.getEmittanceRawData().getShiftPos();
                StringBuilder result = new StringBuilder();
                spaceFormat(result, NUMBERWIDTH, dataFormat.format(0));
                for (int i = 0; i < nx; i++) {                   
                ...
                double stepP = (xpMax - xpMin) / (double) nxp;
                double xP = xpMin+makeRawToEmittancePanel.getEmittanceRawData().getShiftAngle();
                for (int i = 0; i < nxp; i++) {
                    spaceFormat(result, NUMBERWIDTH, dataFormat.format(xP));
                    x = xMin;
                    for (int j = 0; j < nx; j++) {
                        double emx = data.getValue(x, xP);
                        spaceFormat(result, NUMBERWIDTH, dataFormat.format(emx));
                        x += step;
                    }
                    result.append("\n");
                    xP += stepP;
                ...
                Logger.getLogger(RawDataPanel.class.getName()).log(Level.WARNING, null, ex);
            } finally {
                if(bw!=null){
                    try {
                        bw.close();
	...
	private void initiate() {
		dataFormat = new DecimalFormat("0.0000E00");
	...
	private boolean customDistance = false;
	
	public void setDistance(double distance){
		int index = -1;
		if(distance == 0.0){
			index = readRawDataPanel.getTypeIndex();
		}
		makeRawToEmittancePanel.setDistance(distance,index);
