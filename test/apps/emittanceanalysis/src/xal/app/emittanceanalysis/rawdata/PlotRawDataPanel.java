...
public class PlotRawDataPanel {
	...
	private JLabel sampleStart_Label = new JLabel("Start Sample:");
			...
			for (int posInd = posStart; (posInd <= posMax && posInd <= posStop); posInd += posStep) {
				for (int posHInd = posHStart; (posHInd <= posHMax && posHInd <= posHStop); posHInd += posHStep) {
					...
					cd.setColor(IncrementalColors.getColor(index));
                                        ...
                                        if(devType==4){
                                            DataRecord2D record = rawData.getSlitData().getDataRecord(posInd - 1);
                                            double polarity = rawData.getPolarity();
                                            for (int is = 0; is < nSamples; is++) {
                                                    double value = polarity*record.getValue(is) / max_val;
	...
	public void setSpinnerModels(int nChannels, int nPositions_Slit, int nPositions_Harp) {
		...
		posStart_Spinner.setValue(1);
		posStep_Spinner.setValue(nPositions_Slit/100+1);
		posStop_Spinner.setValue(nPositions_Slit);
	 ...
	 */
        ...
        public static int coerce(int start, int end, int value){
            if(value>end){
                return end;
            }
            else if(value<start){
	...
	public void setDefaultSpinnersValues() {
		...
		chStart_Spinner.setValue(new Integer(coerce(1,nCh,nCh / 2 - 3)));
		chStep_Spinner.setValue(new Integer(1));
		chStop_Spinner.setValue(new Integer(coerce(1,nCh,nCh / 2 + 3)));

		posStart_Spinner.setValue(1);
		posStep_Spinner.setValue(nPos/100+1);
