...
import xal.extension.emscan.Factories;
import xal.extension.emscan.FileTimeData;
...
public final class WireRawData {
	...
	private TimeData<DataRecord2D> slitSlitData;

	public TimeData<DataRecord2D> getSlitData() {
		return slitSlitData;
	}
	...
	/**
	 * Reads data. User has to defined how many wires and channels where
	 * used.
	 *
	...
	public boolean readData(File file, int nWiresIn, int nChannelsIn, double data_polarity) {
		...
		if (devType == 4) {
			...
			slitSlitData = new FileTimeData<DataRecord2D>(Factories.getDataRecord2DFactory(), file);
			polarity = data_polarity;
			nWires = 1;
			nChannels = 1;
			nPos_Slit = slitSlitData.size();
			nSamples = slitSlitData.getDataRecord(0).size();
			nPos_Harp = 1;

			slit_pos = new double[nPos_Slit];
			harp_pos = new double[nPos_Slit][nPos_Harp];
			for (int i = 0; i < nPos_Slit; i++) {
				DataRecord2D record = slitSlitData.getDataRecord(i);
				slit_pos[i] = record.getX();
				harp_pos[i][0] = record.getY();
			}
		...
		for (int iw = 0; iw < nWires; iw++) {
			for (int ips = 0; ips < nPos_Slit; ips++) {
				for (int iph = 0; iph < nPos_Harp; iph++) {
					...
					if (devType == 4) {
						DataRecord2D record = slitSlitData.getDataRecord(ips);
						for (int is = 0; is < record.size(); is++) {

							v_tmp = Math.abs(record.getValue(is));
	...
	/**
	 * Reads data from an ASCII file with numbers only. User has to defined
	 * how many wires and channels where used.
	 *
	...
	public double getValue(int sampleInd, int posInd_Slit, int posInd_Harp, int wireInd) {
		...
		return slitSlitData.getDataRecord(posInd_Slit).getValue(sampleInd) * polarity;
	}

	/**
	 * Sets the value of the wire signal for particular sample, position and
	 * wire
	 *
	...
	/**
	 * Returns the maximal value in the sample for particular wire and
	 * position.
	 *
	 * @param posInd_Harp Description of the Parameter
	 * @param posInd_Slit Description of the Parameter
	...
	/**
	 * Returns number of channels. It is less than number of wires, because
	 * wires with indexes 30-31 are used for other purposes.
	 *
	...
	/**
	 * Sets the device type. This index has to be set from the
	 * ReadRawDataPanel class
	 *
