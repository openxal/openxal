...
//    JLabel harplabel = new JLabel("Harp:");
    ...
    private Map<String, JCheckBox> selected_f = new HashMap<>();
    private Map<String, DecimalField>  x_f= new HashMap<>();
    private Map<String, DecimalField> y_f = new HashMap<>();
    private final java.util.List<String> prof_names = new ArrayList<>();
...
//    double harparea=0.0;
    ...
    private HashMap<String, List<Double>> harpRatios = new HashMap<String,  List<Double>>();
    ...
    static private String sortName(String name){
        return name.split(":")[1];
    }
	...
	public void addcomponents(){
        ...
        GridLayout initgrid = new GridLayout(prof_names.size() + 1, 4);
        ...
        for (String id: prof_names){
            String sName = BeamSizeFace.sortName(id);
            DecimalField x = new DecimalField(0.0, 4, numFor);
            x_f.put(id, x);
            DecimalField y = new DecimalField(0.0, 4, numFor);
            y_f.put(id, y);
            JCheckBox selected = new JCheckBox();
            selected.setSelected(true);
            selected_f.put(id, selected);
            initPanel.add(new JLabel(sName)); initPanel.add(x); initPanel.add(y); initPanel.add(selected);
    ...
    public void init(){
        ...
        // 7/6 scaling after adding second harp
        initPanel.setPreferredSize(new Dimension(280,110*7/6));
        ...
        sendtoharpbutton = new JButton("Send Results to Harps");
        ...
        java.util.List<AcceleratorNode> profileNodes = seq.getAllNodesWithQualifier(new OrTypeQualifier().or("WS", "Harp"));

        for (AcceleratorNode n: profileNodes) {
            String id = n.getId();
            prof_names.add(id);
        }
        ...
        tabledata = new Object[prof_names.size()+3][3];
        makeResultsTable();
...
//        tabledata[4][0] = new String("RTBT_Diag:Harp30");
        ...
        int index = 0;
        for (String n: prof_names){
            tabledata[index][0] = n;
            index++;
        }

        tabledata[index][0] = new String("Window");
        index++;
        tabledata[index][0] = new String("Target");
        index++;
        tabledata[index][0] = new String("Target (with scattering)");
...
//
    ...
    public void solve() {
        ...
        int index = 0;
        for (String n: prof_names){
            Object y = y_f.get(n).getValue();
            Object x = x_f.get(n).getValue();
            xfulldatalist.add(x);
		    yfulldatalist.add(y);

            if(selected_f.get(n).isSelected()){
                namelist.add(n);
                xdatalist.add(x);
                ydatalist.add(y);
                if(first) istart = index;
                first = false;
            }
            index++;
        }
...
//        PhaseMatrix matRef = (probe).stateResponse(currentElem, refStart);
    ...
    public void makeResultsTable(){
        ...
        resultstablemodel = new ResultsTableModel(colnames,prof_names.size()+3);
    ...
    public void resetResultsTable(){
        ...
        // Harp area is calculated for Harp30 Only
        Map<String, Double> harpAreas = new HashMap<String, Double>();
        int harp30index = prof_names.indexOf("RTBT_Diag:Harp30");
        for(int i=0; i< prof_names.size(); i++){
            String n = prof_names.get(i);
            if(n.startsWith("RTBT_Diag:Harp")){
                harpAreas.put(n,((Double)tabledata[i][1]).doubleValue()*((Double)tabledata[i][2]).doubleValue());
            }
        }

//		harparea=((Double)tabledata[harp30index][1]).doubleValue()*((Double)tabledata[harp30index][2]).doubleValue();;
		...
		rx = 1000*newtwiss[0].getEnvelopeRadius();
		ry = 1000*newtwiss[1].getEnvelopeRadius();
		tabledata[size][1] = new Double(decfor.format(rx));
		tabledata[size][2] = new Double(decfor.format(ry));
		...
		rx = 1000*newtwiss[0].getEnvelopeRadius();
		ry = 1000*newtwiss[1].getEnvelopeRadius();
		tabledata[size+1][1] = new Double(decfor.format(rx));
		tabledata[size+1][2] = new Double(decfor.format(ry));
		...
		tabledata[size+2][1] = new Double(decfor.format(xtargetwin));
		tabledata[size+2][2] = new Double(decfor.format(ytargetwin));
		...
		double xwindowvalue = ((Double)tabledata[size][1]).doubleValue();
		double ywindowvalue = ((Double)tabledata[size][2]).doubleValue();
		double windowarea = xwindowvalue*ywindowvalue;
		double xtargetvalueraw = ((Double)tabledata[size+1][1]).doubleValue();
		double ytargetvalueraw = ((Double)tabledata[size+1][2]).doubleValue();
		double xtargetvalue = ((Double)tabledata[size+2][1]).doubleValue();
		double ytargetvalue = ((Double)tabledata[size+2][2]).doubleValue();
        ...
        harpRatios = new HashMap<String, List<Double>>();
        for(String key : harpAreas.keySet()){
            double harparea = harpAreas.get(key);
            double targetratio = harparea/targetarea;
            double targetratioraw = harparea/targetarearaw;
            double windowratio = harparea/windowarea;
            harpRatios.put(key, Arrays.asList(targetratio, targetratioraw, windowratio));
            System.out.println(key+ " targetratio= " + targetratio + " windowratio= " + windowratio);
        }
    ...
    public void sendtoharp(){
        ...
        for(String key: harpRatios.keySet()) {
            double targetratio = harpRatios.get(key).get(0);
            double windowratio = harpRatios.get(key).get(2);

            System.out.println("Sending to " + key + " targetratio as " + targetratio);
            System.out.println("Sending to " + key + " windowratio as " + windowratio);
            Channel windowch;
            Channel targetch;
            windowch = ChannelFactory.defaultFactory().getChannel(key + ":WindowSizeRatio_Set");
            targetch = ChannelFactory.defaultFactory().getChannel(key + ":TargetSizeRatio_Set");
            ...
            try {
                ...
                System.err.println("Unable to connect to channel access: " + e.getMessage());
            } catch (Exception e){
                System.err.println("Unable to set process variables: " + e.getMessage());
            }
        }
    }
