...
public class SnapshotCommitController {
    ...
    private SnapshotCommitController( final WindowReference dialogReference ) {
		...
		final PlainDocument commentDocument = new PlainDocument() {
		...
		COMMENT_PANE.setDocument( commentDocument);
		final JButton pvlogButton = (JButton)dialogReference.getView( "PVLogButton" );
		final JLabel logID = (JLabel)dialogReference.getView( "Label" );
		pvlogButton.addActionListener( new ActionListener() {
			public void actionPerformed( final ActionEvent event ) {
				try {
					RemoteLoggingCenter rL = new RemoteLoggingCenter();
					Date startTime = new Date();
					String pvlogComments = startTime.toString();
					int ln = commentDocument.getLength();
					pvlogComments = pvlogComments + " = SCORE = " + commentDocument.getText(0, ln);
					long snapshotId = rL.takeAndPublishSnapshot( "default", pvlogComments);

					if(snapshotId > 0) {
						commentDocument.insertString(ln, " [ID: " +snapshotId+"]\n", null);
						dismissDialog( true );
					} else {
						logID.setText("PV Logger failed");
//						this is for testing only
//						commentDocument.insertString(ln, " [ID: " +123567567+"]\n", null);
					}
