...
public class Restorer implements Runnable, PutListener {
    ...
    private void fireRestores() {
		...
		// Following few lines were introduced with new injection kicker controls in 2024.
		// AFTER(!!!) WF filename is loaded one has to caput 1 to command PV
		// So it processes the list of selected records and enforces that the command PVs are in the end
		// It also always caputs 1 disregarding saved setpoint which most likely is zero.
		// This is achieved with replacing setPointSavedValue by "1".

		List<ScoreRecord> sortedList = new ArrayList<>();
		List<ScoreRecord> commandList = new ArrayList<>();
		for( ScoreRecord record : selectedRecords ) {
			String spPV = record.getSetpointPV();
			if(spPV != null && spPV.endsWith(":setNewPsWf_Cmd") && spPV.startsWith("Ring_Mag:PS_IKick_")){
				record.setValueForKey("1", PVData.spSavedValKey);
				commandList.add(record);
			} else{
				sortedList.add(record);
			}
		}
		sortedList.addAll(commandList);
