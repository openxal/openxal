...
class Statistic_Runner(Runnable):
	...
	def run(self):
		...
		init_start_stop_panel.ring_energy_text.setValue(0.)		
		buffer_size = int(init_start_stop_panel.buffer_size_text.getValue())		
		...
		while(1 < 2):
			...
			eKin_ring = scl_long_tuneup_energy_meter_controller.energyScaler.eKin_Hm_to_P(eKin*1.0e+6)/1.0e+6
			init_start_stop_panel.ring_energy_text.setValue(eKin_ring)	
			if(not res):
				if(messageTextField != null):
...
class InitStartStopStatistics_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		...
		energy_lbl = JLabel("<html>Found Final Linac E<SUB>kin</SUB>[MeV]=<html>",JLabel.RIGHT)
		...
		ring_energy_lbl = JLabel("<html> Ring E<SUB>kin</SUB>[MeV]=<html>",JLabel.RIGHT)
		self.ring_energy_text = DoubleInputTextField(0.,DecimalFormat("####.######"),12)
		...
		energy_panel.add(ring_energy_lbl)
		energy_panel.add(self.ring_energy_text)
		#----- main panel		
