...
class SCL_One_Cavity_Tracker_Model:
	def __init__(self,scl_long_tuneup_controller):
		...
		self.eKin_in = 185.6
		#---- the start energy at SCL cannot be so small! (< 10 MeV)
		if(cav_wrappers[0].eKin_in < 10.):
...
def calculateEneregyVsPhase(cav_wrapper,scl_long_tuneup_controller,bpm_wrappers_good_arr):
	...
	for ip in range(phaseDiffPlot.getNumbOfPoints()):
		...
		for bpm_ind in range(1,len(bpm_wrappers_good_arr)):
			...
			#print "debug bpm=",bpm_wrapper.alias," pos=",bpm_wrapper.pos," phase=",bpm_phase," bpm_phase_guess=",bpm_phase_guess
		...
		while(bad_point_ind >= 0):
			...
			if(bad_point_ind >= 0):
				bad_points_count += 1
				#print "debug ======== ip=",ip," cav_phase=",cav_phase," bad_points_count=",bad_points_count
		...
		for cav_table_ind in range(start_ind,last_ind+1):
			...
			if(not cav_wrapper.isGood):
				...
				if(cav_table_ind != 0):
					...
					cav_wrapper.bpm_eKin_out = cav_wrappers[cav_table_ind-1].bpm_eKin_out
			...
			if(len(bpm_wrappers_good_arr) < 3):
				if(messageTextField != null):
					...
					analysis_state_controller.setIsRunning(false)
				return						
			result_info = calculateEneregyVsPhase(cav_wrapper,self.scl_long_tuneup_controller,bpm_wrappers_good_arr)
			if(not result_info):
				messageTextField.setText("Cavity "+cav_wrapper.alias+" does not have a good eKinOut estimation! Stop analysis!")
				analysis_state_controller.setIsRunning(false)
				return					
			...
			while(bad_point_ind >= 0):
				...
				for ibp in range(cav_wrapper.eKinOutPlot.getNumbOfPoints()):
					...
					#print "debug index=",ibp," EkinOut=",exp_val," harm_val=",harm_val," diff=",diff," max-diff=",2.8*cav_wrapper.eKin_err
					if(math.fabs(exp_val-harm_val) > 2.8*cav_wrapper.eKin_err):
						bad_point_ind = ibp
						break
				if(bad_point_ind >= 0):
					#print "debug bad point bad_point_ind = ",bad_point_ind," bad_points_count = ",bad_points_count 
					bad_points_count += 1 
					#print "debug ==========  point bad_point_ind = ",bad_point_ind," bad_points_count = ",bad_points_count 
			...
			#print "debug ==========  cav_wrapper.bpm_eKin_out=",cav_wrapper.bpm_eKin_out
...
class Cav_Energy_Out_Graph_Panel(JPanel):
	...
	def updateGraphData(self):
		...
		for ind in cav_selected_inds:
			...
			for ibp in range(cav_wrapper.eKinOutPlotTh.getNumbOfPoints()):
				print cav_wrapper.eKinOutPlotTh.getX(ibp), cav_wrapper.eKinOutPlotTh.getY(ibp)
...
class PostPhaseAnalysisActions_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		...
		export_cavs_energy_vs_phase_button = JButton("Export Energy vs. Phase to ASCII")
		export_cavs_energy_vs_phase_button.addActionListener(Export_Enrg_vs_Phase_to_ASCII_Listener(self.scl_long_tuneup_controller))		
		...
		post_analysis_panel.add(export_cavs_energy_vs_phase_button)
		#--------------------------
...
class Export_Table_to_ASCII_Listener(ActionListener):
	...
	def actionPerformed(self,actionEvent):
		...
		if(returnVal == JFileChooser.APPROVE_OPTION):
			...
			txt += " delta_eKin_in_out_keV bpm_eKin_out model_eKin_out delta_eKin_fit_keV  E0TL_MeV phi_EPICS"
			...
			for cav_ind in range(len(cav_wrappers)):
				...
				txt += " %12.5g "%E0TL + "%+6.4f"%cav_wrapper.livePhase
				buffer_out.write(txt)
				buffer_out.newLine()
			#---- end of writing
			buffer_out.flush()
...
class Export_Enrg_vs_Phase_to_ASCII_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
	...
	def actionPerformed(self,actionEvent):
		self.scl_long_tuneup_controller.getMessageTextField().setText("")		
		fc = JFileChooser(constants_lib.const_path_dict["LINAC_WIZARD_FILES_DIR_PATH"])
		fc.setDialogTitle("Save SCL Energy vs Phase data into ASCII file")
		fc.setApproveButtonText("Save")
		fc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY)
		fc.setAcceptAllFileFilterUsed(false)
		returnVal = fc.showOpenDialog(self.scl_long_tuneup_controller.linac_wizard_document.linac_wizard_window.frame)
		if(returnVal == JFileChooser.APPROVE_OPTION):
			dir_out = fc.getSelectedFile()
			cav_wrappers = self.scl_long_tuneup_controller.cav_wrappers
			file_name = str(dir_out) +"/scl_rf_phase_scans_ekin_vs_phase.dat"
			file_out = open(file_name,"w")
			for cav_ind in range(len(cav_wrappers)):
				cav_wrapper = cav_wrappers[cav_ind]
				#---- print the OpenXAL analysis information
				txt = cav_wrapper.cav.getId()+" pos = %8.3f "%cav_wrapper.pos+" initLiveAmp = %12.5g "%cav_wrapper.initLiveAmp
				txt += " livePhase = %10.5f "%cav_wrapper.livePhase
				txt += " designPhase1stGap = %10.5f "%cav_wrapper.designPhase 
				txt += " avg_gap_phase = %10.5f "%cav_wrapper.avg_gap_phase + " scanPhaseShift =  %10.5f "%cav_wrapper.scanPhaseShift
				txt += " real_scanPhaseShift = %10.5f "%cav_wrapper.real_scanPhaseShift
				txt += " eKin_in = %12.6g "%cav_wrapper.eKin_in + " eKin_out = %12.6g "%cav_wrapper.eKin_out
				txt += " bpm_eKin_out = %12.6g "%cav_wrapper.bpm_eKin_out+ " model_eKin_out = %12.6g "%cav_wrapper.model_eKin_out
				txt += " eKin_err = % 6.1f"%(1000.*cav_wrapper.eKin_err)
				E0TL = 0.
				if(len(cav_wrapper.energy_guess_harm_funcion.getParamArr()) > 1):
					E0TL = cav_wrapper.energy_guess_harm_funcion.getParamArr()[1]
				txt += " E0TL = %12.5g "%E0TL
				file_out.write(txt + "\n")
				#---- print eKin_Out as a function of cavity phase
				txt = " phase[deg]   eKin[MeV]  eKinTheory[MeV] delta[keV] "
				file_out.write(txt + "\n")
				eKin_gd = cav_wrapper.eKinOutPlot
				eKinTh_gd = cav_wrapper.eKinOutPlotTh
				for ind in range(eKin_gd.getNumbOfPoints()):
					phase = eKin_gd.getX(ind)
					eKin = eKin_gd.getY(ind)
					eKinTh = eKinTh_gd.getValueY(phase)
					diff = (eKin - eKinTh)*1000
					txt = " %10.5f  %14.8g  %14.8g  %10.3g "%(phase,eKin,eKinTh,diff)
					file_out.write(txt + "\n")
			file_out.close()
