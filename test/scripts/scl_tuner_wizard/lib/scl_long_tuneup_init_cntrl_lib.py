...
class SCL_Quad_Fields_Dict_Holder:
	def __init__(self,scl_long_tuneup_controller):
		...
		for quad in quads:
			...
			self.quad_new_field_dict[quad] = field
			...
			if(cav_wrapper.isGood):
				...
				if(fake_scan):
					...
					if False and (cav.getId().find("23d") >= 0 or int(cav.getId()[10:12]) >= 31):
						phase_tmp = - phase_tmp
					cav_wrapper.initLivePhase = phase_tmp
		...
		for cav_wrapper in cav_wrappers:
			if(cav_wrapper.isGood and cav_wrapper.initLiveAmp > alive_cavity_amp_limit):
				...
				if(fake_scan):
					...
					phase_tmp = cav_wrapper.initLivePhase
					if False and (cav.getId().find("23d") >= 0 or int(cav.getId()[10:12]) >= 31):
						phase_tmp = - phase_tmp			
					cav.setCavPhase(phase_tmp)
		#---- update bpm and cavity tables
...
class Update_Selected_Cav_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
...
class Remove_Selected_BPM_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
	...
	def actionPerformed(self,actionEvent):
		...
		bpm_selected_ind = bpm_table.getSelectedRow()
		if(bpm_selected_ind < 0): 
			messageTextField.setText("There is no selected BPM!")
			return
		bpm_wrapper_selected = self.scl_long_tuneup_controller.bpm_wrappers[bpm_selected_ind]
		cav_wrappers = self.scl_long_tuneup_controller.cav_wrappers
		st_wrong_cav_names = ""
		for cav_wrapper_ind in range(len(cav_wrappers)):
			cav_wrapper = cav_wrappers[cav_wrapper_ind]
			if(bpm_wrapper_selected == cav_wrapper.bpm_wrapper0 or bpm_wrapper_selected == cav_wrapper.bpm_wrapper1):
				st_wrong_cav_names += " "+cav_wrapper.alias+","
		if(st_wrong_cav_names != ""):
			messageTextField.setText("BPM"+bpm_wrapper_selected.alias+" is used by cavs.:" + st_wrong_cav_names+" Fix it!")
			return			
		for cav_wrapper_ind in range(len(cav_wrappers)):
			cav_wrapper = cav_wrappers[cav_wrapper_ind]
			cav_wrapper.removeBPM_Wrapper(bpm_wrapper_selected)
		self.scl_long_tuneup_controller.cav0_wrapper.removeBPM_Wrapper(bpm_wrapper_selected)
		bpm_wrapper_selected.isGood = false
 		bpm_table.getModel().fireTableDataChanged()		
...
class Read_Synch_Cav_Phases_Button_Listener(ActionListener):
	...
	def actionPerformed(self,actionEvent):
		...
		fc.setDialogTitle("Read Target Synch. Phases from ASCII file")
		...
		if(returnVal == JFileChooser.APPROVE_OPTION):
			...
			for cav_wrapper in cav_wrappers:
				...
				if(cav_synch_pahses_dict.has_key(cav_name)):
					cav_wrapper.scanPhaseShift = cav_synch_pahses_dict[cav_name]
...
class SCL_Long_TuneUp_Init_Controller:
	def __init__(self,scl_long_tuneup_controller):
		...
		#--------------------------------------------
		update_slected_button = JButton(" Update Phase & Amp of Selected Cavity ")
		remove_selected_bpm_button = JButton(" Remove Selected BPM from Use ")
		read_cav_synch_phases_button = JButton(" Read Target Synch Phases from ASCII ")
		update_slected_button.addActionListener(Update_Selected_Cav_Button_Listener(self.scl_long_tuneup_controller))
		remove_selected_bpm_button.addActionListener(Remove_Selected_BPM_Button_Listener(self.scl_long_tuneup_controller))
		read_cav_synch_phases_button.addActionListener(Read_Synch_Cav_Phases_Button_Listener(self.scl_long_tuneup_controller))
		init_panel02 = JPanel(GridLayout(4,1,10,10))
		init_panel02.add(update_slected_button)
		init_panel02.add(remove_selected_bpm_button)
		init_panel02.add(read_cav_synch_phases_button)
		#--------------------------------------------
		init_panel0.add(init_panel01,BorderLayout.WEST)
		init_panel0.add(init_panel02,BorderLayout.EAST)
