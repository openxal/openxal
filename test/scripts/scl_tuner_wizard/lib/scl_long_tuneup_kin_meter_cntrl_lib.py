...
#------------------------------------------------------------------------	
class KinStatisticStateController:
	def __init__(self):
...
class Kin_BPMs_Wrappers_Holder:
	...
	def setBPM_Wrappers(self):
		...
		scl_long_tuneup_kin_meter_controller = self.scl_long_tuneup_controller.scl_long_tuneup_kin_meter_controller
		init_start_stop_panel = scl_long_tuneup_kin_meter_controller.init_start_stop_panel
		blankCavities_RadioButton = init_start_stop_panel.blankCavities_RadioButton
		bpm_wrappers = self.scl_long_tuneup_controller.bpm_wrappers
		cav_wrappers = self.scl_long_tuneup_controller.cav_wrappers
		cav_wrapper = cav_wrappers[0]
		max_pos = 280.0
		self.clean()
		for bpm_wrapper in bpm_wrappers:
			if(bpm_wrapper.isGood and bpm_wrapper.pos < max_pos):
				self.em_bpm_wrpprs.append(bpm_wrapper)
				phase_offset = 0.
				isReady = bpm_wrapper.final_phase_offset.isReady
				if(isReady and blankCavities_RadioButton.isSelected()):
					phase_offset = bpm_wrapper.final_phase_offset.phaseOffset_avg
					self.use_bpms.append(true)
				elif(isReady and (not blankCavities_RadioButton.isSelected()) and bpm_wrapper.pos < cav_wrapper.pos):
					phase_offset = bpm_wrapper.final_phase_offset.phaseOffset_avg
					self.use_bpms.append(true)
				else:
	...
	def calculateEnergy(self,eKin_in):
		scl_long_tuneup_kin_meter_controller = self.scl_long_tuneup_controller.scl_long_tuneup_kin_meter_controller
		table_and_plots_panel = scl_long_tuneup_kin_meter_controller.table_and_plots_panel
		init_start_stop_panel = scl_long_tuneup_kin_meter_controller.init_start_stop_panel
		buffer_size = int(init_start_stop_panel.buffer_size_text.getValue())
		bpm_amp_thr = init_start_stop_panel.bpm_amp_thr_text.getValue()
		...
		phase_avg0 = phase_avg0 - self.bpm_phase_offsets[bpm_ind0]
...
class Kin_Statistic_Runner(Runnable):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
		#------------------------------------------------------------
		self.bpm_wrappers_good_arr = []
		scl_long_tuneup_kin_meter_controller = self.scl_long_tuneup_controller.scl_long_tuneup_kin_meter_controller
		bpm_wrappers_holder = scl_long_tuneup_kin_meter_controller.bpm_wrappers_holder
	...
	def run(self):
		...
		for ind in range(len(bpm_wrappers_holder.em_bpm_wrpprs)):
			bpm_wrapper = bpm_wrappers_holder.em_bpm_wrpprs[ind]
			print bpm_wrapper.alias, bpm_wrappers_holder.use_bpms[ind]
		"""
		messageTextField = self.scl_long_tuneup_controller.getMessageTextField()
		if(messageTextField != null):
			messageTextField.setText("")	
		scl_long_tuneup_kin_meter_controller = self.scl_long_tuneup_controller.scl_long_tuneup_kin_meter_controller
		statistic_state_controller = scl_long_tuneup_kin_meter_controller.statistic_state_controller
		init_start_stop_panel = scl_long_tuneup_kin_meter_controller.init_start_stop_panel
		...
		if(init_start_stop_panel.energy_guess_text.getValue() == 0.):
			if(messageTextField != null):
				...
				return				
		bpm_table = scl_long_tuneup_kin_meter_controller.table_and_plots_panel.bpm_table
		counter_text = init_start_stop_panel.counter_text
		counter_text.setValue(0.)

		cav_wrappers = self.scl_long_tuneup_controller.cav_wrappers
		blankCavities_RadioButton = init_start_stop_panel.blankCavities_RadioButton
		if(blankCavities_RadioButton.isSelected()):
		...
		scl_long_tuneup_kin_meter_controller.table_and_plots_panel.bpm_phase_err_gd.removeAllPoints()
		bpm_wrappers_holder = scl_long_tuneup_kin_meter_controller.bpm_wrappers_holder
...
#           Auxiliary panels
#------------------------------------------------------------------------		
class KinInitStartStopStatistics_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
		...
		init_meter_button.addActionListener(Kin_Init_Meter_Button_Listener(self.scl_long_tuneup_controller))		
		start_statistics_button = JButton("Start Statistic")
		start_statistics_button.addActionListener(Kin_Start_Statistics_Button_Listener(self.scl_long_tuneup_controller))
		stop_statistics_button = JButton("Stop")
		stop_statistics_button.addActionListener(Kin_Stop_Statistics_Button_Listener(self.scl_long_tuneup_controller))		
		...
		bpm_amp_thr_lbl = JLabel("  BPM Amplitude Threshold",JLabel.RIGHT)
		self.bpm_amp_thr_text = DoubleInputTextField(2.,DecimalFormat("##.#"),4)
		self.blankCavities_RadioButton = JRadioButton("Blank all cavities")	
		self.blankCavities_RadioButton.setSelected(true)
		...
		buttons_panel.add(bpm_amp_thr_lbl)
		buttons_panel.add(self.bpm_amp_thr_text)
		buttons_panel.add(self.blankCavities_RadioButton)
		#----- energy panel
		...
		self.energy_guess_text = DoubleInputTextField(186.,DecimalFormat("####.######"),12)
...
class Kin_Table_and_Plots_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
		self.setLayout(BorderLayout())
		#----etched border
		...
		self.setBorder(titled_border)	
		#----------------------------------------
		self.bpm_table = JTable(Kin_Meter_BPMs_Table_Model(self.scl_long_tuneup_controller))
...
class Kin_Manipulate_Offsets_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
		self.setLayout(BorderLayout())
		#----etched border
...
#		zero_offsets_button.addActionListener(Zero_Offsets_Button_Listener(self.scl_long_tuneup_controller))		
		...
		correct_offsets_button.addActionListener(Kin_Correct_Offsets_Diff_Button_Listener(self.scl_long_tuneup_controller))
		write_to_ascii_button = JButton("Write Offsets to ASCII")
		write_to_ascii_button.addActionListener(Kin_Write_To_ASCII_Button_Listener(self.scl_long_tuneup_controller))
		read_from_ascii_button = JButton("Read Offsets from ASCII")
		read_from_ascii_button.addActionListener(Kin_Read_From_ASCII_Button_Listener(self.scl_long_tuneup_controller))
...
#  JTable models
#------------------------------------------------
class Kin_Meter_BPMs_Table_Model(AbstractTableModel):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
...
#           Listeners
#------------------------------------------------------------------------
class Kin_Init_Meter_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
	...
	def actionPerformed(self,actionEvent):
		...
		init_start_stop_panel.energy_guess_text.setValue(eKin_in)
		init_start_stop_panel.energy_err_text.setValue(0.)
		table_and_plots_panel = scl_long_tuneup_kin_meter_controller.table_and_plots_panel
		table_and_plots_panel.bpm_table.getModel().fireTableDataChanged()

class Kin_Start_Statistics_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
	...
	def actionPerformed(self,actionEvent):	
		...
		statistic_state_controller = scl_long_tuneup_kin_meter_controller.statistic_state_controller
		if(statistic_state_controller.getIsRunning()): return
		statistic_runner = Kin_Statistic_Runner(self.scl_long_tuneup_controller)
		thr = Thread(statistic_runner)
		thr.start()				

class Kin_Stop_Statistics_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
	...
	def actionPerformed(self,actionEvent):	
		self.scl_long_tuneup_controller.getMessageTextField().setText("")
		scl_long_tuneup_kin_meter_controller = self.scl_long_tuneup_controller.scl_long_tuneup_kin_meter_controller
		scl_long_tuneup_kin_meter_controller.statistic_state_controller.setShouldStop(true)

"""		
class Kin_Zero_Offsets_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller		
...
class Kin_Correct_Offsets_Diff_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
...
class Kin_Write_To_ASCII_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
...
class Kin_Read_From_ASCII_Button_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
...
#           Controllers
#------------------------------------------------------------------------
class SCL_Kin_Meter_Controller:
	def __init__(self,scl_long_tuneup_controller):
		...
		etched_border = BorderFactory.createEtchedBorder()	
		#----- auxilary classes 
		self.bpm_wrappers_holder = Kin_BPMs_Wrappers_Holder(self.scl_long_tuneup_controller)
		#------top params panel-----------------------
		...
		#--------------------------------------------------
		self.init_start_stop_panel = KinInitStartStopStatistics_Panel(self.scl_long_tuneup_controller)
		top_panel.add(self.init_start_stop_panel,BorderLayout.NORTH)
		self.table_and_plots_panel = Kin_Table_and_Plots_Panel(self.scl_long_tuneup_controller)
		center_panel.add(self.table_and_plots_panel,BorderLayout.CENTER)
		self.manipulate_offsets_panel = Kin_Manipulate_Offsets_Panel(self.scl_long_tuneup_controller)
		bottom_panel.add(self.manipulate_offsets_panel,BorderLayout.NORTH)
		#------ Statistic run controller
		self.statistic_state_controller = KinStatisticStateController()
		#--------------------------------------------------
	...
	def writeDataToXML(self,root_da):
		if(self.isWorthToSave()):
			energy_meter_da = root_da.createChild("Long_Tune_Kin_Meter_Data")
	...
	def readDataFromXML(self,root_da):
		...
		energy_meter_da = root_da.childAdaptor("Long_Tune_Kin_Meter_Data")
