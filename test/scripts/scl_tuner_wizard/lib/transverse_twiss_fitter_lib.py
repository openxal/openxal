...
class AccScoreCalculator(Scorer):
	...
	def runErrorsCalculator(self):
		...
		#----- calculation emittance error with all covariance parameters from CovMatrix
		sig2_x2 = errorMatrix_X.get(0,0)
		sig2_x_xp = errorMatrix_X.get(1,1)
		sig2_xp2 = errorMatrix_X.get(2,2)
		sig_x2_x_xp = errorMatrix_X.get(0,1)
		sig_x2_xp2 = errorMatrix_X.get(0,2)
		sig_xp2_x_xp = errorMatrix_X.get(2,1)
		emitx2 = math.fabs(x2*xp2-x_xp**2)
		sig2_emitx2 = math.fabs(x2**2*sig2_xp2 + xp2**2*sig2_x2 + 4*x_xp**2*sig2_x_xp + 2*x2*xp2*sig_x2_xp2 - 4*x_xp*(xp2*sig_x2_x_xp - x2*sig_xp2_x_xp))
		emitErrX = 0.5*math.sqrt(sig2_emitx2)/math.sqrt(emitx2)
		print "===============================New emittance error formula for X============================"
		"""
		print "======= Cov Matrix X ======"
		for ind_row in range(3):
			st_ln = ""
			for ind_col in range(3):
				st = " m("+str(ind_row)+","+str(ind_col)+")= %+12.7g "%errorMatrix_X.get(ind_row,ind_col)
				st_ln += st
			print st_ln
		print "   =============== "
		"""
		print "emitt X =",math.sqrt(emitx2)," +- ",emitErrX
		final_twiss_params_holder.setParamsErr(0,alphaX_err, betaX_err, emitErrX)
		#--------------------------------------------------------------------------
		sig2_y2 = errorMatrix_Y.get(0,0)
		sig2_y_yp = errorMatrix_Y.get(1,1)
		sig2_yp2 = errorMatrix_Y.get(2,2)
		sig_y2_y_yp = errorMatrix_Y.get(0,1)
		sig_y2_yp2 = errorMatrix_Y.get(0,2)
		sig_yp2_y_yp = errorMatrix_Y.get(2,1)
		emity2 = math.fabs(y2*yp2-y_yp**2)
		sig2_emity2 = math.fabs(y2**2*sig2_yp2 + yp2**2*sig2_y2 + 4*y_yp**2*sig2_y_yp + 2*y2*yp2*sig_y2_yp2 - 4*y_yp*(yp2*sig_y2_y_yp - y2*sig_yp2_y_yp))
		emitErrY = 0.5*math.sqrt(sig2_emity2)/math.sqrt(emity2)
		"""
		print "===============================New emittance error formula for Y============================"
		print "======= Cov Matrix Y ======"
		for ind_row in range(3):
			st_ln = ""
			for ind_col in range(3):
				st = " m("+str(ind_row)+","+str(ind_col)+")= %+12.7g "%errorMatrix_Y.get(ind_row,ind_col)
				st_ln += st
			print st_ln
		print "   =============== "
		"""
		print "emitt Y =",math.sqrt(emity2)," +- ",emitErrY		
		final_twiss_params_holder.setParamsErr(1,alphaY_err, betaY_err, emitErrY)
		#--------------------------------------------------------------------------
		init_and_fit_params_controller.finalTwiss_table.getModel().fireTableDataChanged()		
