...
class PhaseScan_Runner(Runnable):
	...
	def run(self):
		...
		#--- BPMs timing prediction related variables
		use_bpm_timing_predictions = scl_long_tuneup_phase_scan_controller.set_phase_shift_panel.useBPM_TimingEstimateButton.isSelected()
		skip_cavs_for_bpm_timing = scl_long_tuneup_phase_scan_controller.set_phase_shift_panel.useBPM_Timing_SkipCavs_text.getValue()
		bpm_timing_calculator = self.scl_long_tuneup_controller.bpm_time_shift_scan_conroller.bpm_timing_calculator
		#--------------------------------------------
		...
		for cav_table_ind in range(start_ind,last_ind+1):
			#-------------------------Set BPMs OEDA Timing --------------------------
			#--- OEDA stands for Off Energy Delay Adjustment
			if(use_bpm_timing_predictions and cav_table_ind % skip_cavs_for_bpm_timing == 0):
				bpm_timing_calculator.calculateAndSetBPMsOEDA_Timing()
			#-----------------------------------------------------------------------
	...
	def setUpCavityPhase(self,cav_wrapper):
		...
		if(not self.fake_scan):
			cav = cav_wrapper.cav
			phase_tmp = cav_wrapper.livePhase
			if False and (cav.getId().find("23d") >= 0 or int(cav.getId()[10:12]) >= 31):
				phase_tmp = - phase_tmp
			cav_wrapper.cav.setCavPhase(phase_tmp)
	...
	def allCavitiesOffAnalysis(self,cav_wrapper):
		...
		nBPMs_for_Cntrl = int(scl_long_tuneup_phase_scan_controller.set_phase_shift_panel.numberOfBPMs_for_AllRF_Off_text.getValue())
		bpm_wrappers_avgPhase_err_arr = []
		for bpm_wrapper_ind in range(len(cav_wrapper.bpm_wrappers[0:nBPMs_for_Cntrl])):
...
class SetPhaseShiftAndTimeStep_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		...
		self.useBPM_TimingEstimateButton = JRadioButton("Use BPMs' Timing")
		self.useBPM_Timing_SkipCavs_text = DoubleInputTextField(5.0,DecimalFormat("#"),2)
		useBPM_Timing_SkipCavs_label = JLabel("Skip Cavs # =",JLabel.LEFT)
		numberOfBPMs_for_AllRF_Off_Quality_Cntrl_label = JLabel("  # BPMs for All RF Off =",JLabel.LEFT)
		self.numberOfBPMs_for_AllRF_Off_text = DoubleInputTextField(5,DecimalFormat("#"),2)
		...
		self.useBPM_TimingEstimateButton.setSelected(True)
		...
		sub2_panel.add(self.useBPM_TimingEstimateButton)
		sub2_panel.add(useBPM_Timing_SkipCavs_label)
		sub2_panel.add(self.useBPM_Timing_SkipCavs_text)
		sub2_panel.add(numberOfBPMs_for_AllRF_Off_Quality_Cntrl_label)
		sub2_panel.add(self.numberOfBPMs_for_AllRF_Off_text)
