...
public class XmlDataAdaptor implements DataAdaptor {
    ...
    /** Delete existing offspring DataAdaptors defined by the tagName (T. Gorlov, 07/17/2020) */
    public void removeChildren(String tagName) { 
        
        List<Node> removeChilds = new ArrayList<Node>();
        for ( Node node : childNodes ) {
            if ( nameForNode( node ).equals( tagName ) ) {               
                removeChilds.add(node);

            }

            }   
        
        for ( Node node : removeChilds) {
