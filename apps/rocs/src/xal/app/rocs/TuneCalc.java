...
// based on the user given x and y cords that are given
// by the user. This class required the following parameters
// to be passed durring instatiation:
// xi, yj -- indecies of the desired point (found in TuneSearch.java)
// inx, iny -- point that the user is looking for (entered in TuneFace.java)
//
/**********************************************************/
...
public class TuneCalc extends TuneSettings{
    ...
    //interpolation for each magnet class k1-k6
	...
	if(ptx == tune_x[imax][yj]){   //Initialize delta variables
	    deltax = 0.0;              //to zero if the user input
	}                              //number is the max to be used
	else{
	    distx = tune_x[xi+1][yj] - ptx;  
	    deltax =  (inx - tune_x[xi][yj]) / distx;
	}
	if(pty == tune_y[xi][jmax]){
	    deltay= 0.0;
	}
	else{
	    disty = tune_y[xi][yj+1] - pty;
	    deltay =  (iny - tune_y[xi][yj]) / disty;
	}
