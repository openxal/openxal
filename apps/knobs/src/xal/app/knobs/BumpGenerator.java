...
//		final PlaneAdaptor planeAdaptor = _planeAdaptor;
	...
	protected double[] getBaseOrbit( final AcceleratorNode bumpNode, final AcceleratorNode endNode ) {
		...
		final Trajectory<? extends ProbeState<?>> trajectory = _baseTrajectory;
...
//		final IPhaseState endState = (IPhaseState)trajectory.statesForElement( endNode.getId() )[0];
        final ProbeState<? extends ProbeState<?>> bumpState = trajectory.statesForElement( bumpNode.getId() ).get(0);
        final ProbeState<? extends ProbeState<?>> endState = trajectory.statesForElement( endNode.getId() ).get(0);
	...
	protected double[] calculateResponse( final AcceleratorNode bumpNode, final AcceleratorNode endNode, final Dipole magnet, final double amplitude ) {
		...
		try {
			...
			final Trajectory<? extends ProbeState<?>> trajectory = _probe.getTrajectory();
