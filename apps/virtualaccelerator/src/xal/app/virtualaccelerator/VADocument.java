...
import xal.tools.plot.FunctionGraphsPopupAdaptor;
 ...
 * @author Paul Chu
 */
public class VADocument extends AcceleratorDocument implements ActionListener,
		PutListener {
	...
	public VADocument(final java.net.URL url) {
		...
		// queue to synchronize readbacks with setpoints as well as the online
		// model
		final DispatchQueue modelSyncQueue = DispatchQueue
				.createSerialQueue("Model Sync Queue");
		MODEL_SYNC_TIMER = DispatchTimer.getCoalescingInstance(
				DispatchQueue.createSerialQueue(""),
				getOnlineModelSynchronizer());
		...
		final WindowReference windowReference = getDefaultWindowReference(
				"MainWindow", this);
		_windowReference = windowReference;

		READBACK_SET_TABLE_MODEL = new KeyValueFilteredTableModel<ReadbackSetRecord>(
				new ArrayList<ReadbackSetRecord>(), "node.id",
				"readbackChannel.channelName", "lastReadback",
				"setpointChannel.channelName", "lastSetpoint");
		...
		READBACK_SET_TABLE_MODEL.setColumnName("readbackChannel.channelName",
				"Readback PV");
		READBACK_SET_TABLE_MODEL.setColumnName("lastReadback", "Readback");
		READBACK_SET_TABLE_MODEL.setColumnName("setpointChannel.channelName",
				"Setpoint PV");
		READBACK_SET_TABLE_MODEL.setColumnName("lastSetpoint", "Setpoint");
		READBACK_SET_TABLE_MODEL.setColumnEditable("lastSetpoint", true);

		final JTextField filterField = (JTextField) windowReference
				.getView("FilterField");
		...
		_probeFileTracker = new RecentFileTracker(1, this.getClass(),
				"recent_probes");
	...
	public void makeMainWindow() {
		...
		final JTable readbackTable = (JTable) _windowReference
				.getView("ReadbackTable");
		readbackTable.setCellSelectionEnabled(true);
		readbackTable.setModel(READBACK_SET_TABLE_MODEL);

		makeNoiseDialog();
		/** add digaplot */
		final FunctionGraphsJPanel bpmplot = (FunctionGraphsJPanel) _windowReference
				.getView("BPMPlot");
		final FunctionGraphsJPanel wsplot = (FunctionGraphsJPanel) _windowReference
				.getView("WSPlot");
		_diagplot = new DiagPlot(bpmplot, wsplot);
	...
	private void createDefaultProbe() {
		if (selectedSequence != null) {
			try {
				myProbe = (selectedSequence instanceof xal.smf.Ring) ? createRingProbe(selectedSequence)
						: createEnvelopeProbe(selectedSequence);

				if (selectedSequence instanceof xal.smf.Ring) {
					final TransferMapState state = (TransferMapState) myProbe
							.createProbeState();
					// set initial x, y slightly off-axis to introduce betatron
					// oscillation.
					state.setPhaseCoordinates(new PhaseVector(0.01, 0., 0.01,
							0., 0., 0.));
	...
	static private Probe createRingProbe(final AcceleratorSeq sequence)
			throws InstantiationException {
		final TransferMapTracker tracker = AlgorithmFactory
				.createTransferMapTracker(sequence);
		return ProbeFactory.getTransferMapProbe(sequence, tracker);
	}

	/** create a new envelope probe */
	static private Probe createEnvelopeProbe(final AcceleratorSeq sequence)
			throws InstantiationException {
		final EnvelopeTracker tracker = AlgorithmFactory
				.createEnvelopeTracker(sequence);
	...
	protected void customizeCommands(Commander commander) {
		...
		Action openprobeAction = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent event) {
				final String currentDirectory = _probeFileTracker
						.getRecentFolderPath();
		...
		Action probeEditorAction = new AbstractAction("probe-editor") {
			...
			public void actionPerformed(ActionEvent event) {
				displayError("Probe Editor Error",
						"Probe Editor is not implemented.");
			...
			public void actionPerformed(ActionEvent event) {
				if (vaRunning) {
					JOptionPane.showMessageDialog(getMainWindow(),
							"Virtual Accelerator has already started.",
				...
				if (getSelectedSequence() == null) {
					JOptionPane.showMessageDialog(getMainWindow(),
							"You need to select sequence(s) first.",
							"Warning!", JOptionPane.PLAIN_MESSAGE);
				} else {
					// use PV logger
					if (isFromPVLogger) {
						...
						if (isForOLM) {
							...
							MODEL_SYNC_TIMER
									.setEventHandler(getOnlineModelSynchronizer());
						} else { // directly use PVLogger data for replay
							MODEL_SYNC_TIMER
									.setEventHandler(getPVLoggerSynchronizer());
						}
					}
					// use online model
					else {
						if (theProbeFile == null && myProbe == null) {
							...
							if (myProbe == null) {
								displayWarning("Warning!",
										"You need to select probe file first.");
								return;
							}
		...
		final Action synchConfigAction = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent event) {
				final String result = JOptionPane
						.showInputDialog(
								getMainWindow(),
								"Set the Model Synchronization Period (milliseconds): ",
								_modelSyncPeriod);
				if (result != null) {
					try {
						...
						displayError("Error setting Model Sync Period!",
	...
	public void update(final DataAdaptor adaptor) {
		if (getSource() != null) {
			...
			try {
				...
				this.setAccelerator(dMgr.getAccelerator(),
				...
				JOptionPane
						.showMessageDialog(
								...
								"VA error", JOptionPane.ERROR_MESSAGE);
			}
			...
			if (da1.hasAttribute("env_probe")) {
				...
				if (theProbeFile.length() > 1) {
					try {
						myProbe = (EnvelopeProbe) ProbeXmlParser
								.parse(theProbeFile);
					} catch (ParsingException e) {
						// if we have trouble restore the probe, just create
						// from default
						createDefaultProbe();
					}
				}
	...
	private void updateLimitChannels() {
		for (final Electromagnet magnet : mags) {
			try {
				final Channel bookChannel = magnet.getMainSupply().findChannel(
						MagnetMainSupply.FIELD_BOOK_HANDLE);
				final Channel fieldChannel = magnet.getMainSupply()
						.findChannel(MagnetMainSupply.FIELD_SET_HANDLE);
				if (bookChannel != null) {
					if (bookChannel.isConnected()) {
						...
						final String[] warningPVs = fieldChannel
								.getWarningLimitPVs();

						final Channel lowerWarningChannel = ChannelFactory
								.defaultFactory().getChannel(warningPVs[0],
										fieldChannel.getValueTransform());
						// System.out.println( "Lower Limit PV: " +
						// lowerWarningChannel.channelName() );
						if (lowerWarningChannel.connectAndWait()) {
							lowerWarningChannel.putValCallback(bookField
									- warningOffset, this);
						}

						final Channel upperWarningChannel = ChannelFactory
								.defaultFactory().getChannel(warningPVs[1],
										fieldChannel.getValueTransform());
						if (upperWarningChannel.connectAndWait()) {
							upperWarningChannel.putValCallback(bookField
									+ warningOffset, this);
						}

						final String[] alarmPVs = fieldChannel
								.getAlarmLimitPVs();

						final Channel lowerAlarmChannel = ChannelFactory
								.defaultFactory().getChannel(alarmPVs[0],
										fieldChannel.getValueTransform());
						if (lowerAlarmChannel.connectAndWait()) {
							lowerAlarmChannel.putValCallback(bookField
									- alarmOffset, this);
						}

						final Channel upperAlarmChannel = ChannelFactory
								.defaultFactory().getChannel(alarmPVs[1],
										fieldChannel.getValueTransform());
						if (upperAlarmChannel.connectAndWait()) {
							upperAlarmChannel.putValCallback(bookField
									+ alarmOffset, this);
						}
					}
				}
	...
	private void putReadbackPVs() {
		...
		try {
			...
			if (_repRateChannel != null) {
				final double updatePeriod = 0.001 * (now.getTime() - _lastUpdate
						.getTime()); // period of update in seconds
		...
		// get the "set" PV value, add noise, and then put to the corresponding
		// readback PV.
	...
	private void putReadbackPVsFromPVLogger() {
		...
		for (final ReadbackSetRecord record : READBACK_SET_RECORDS) {
			try {
				final String readbackPV = record.getReadbackChannel()
						.channelName();

				if (qPVMap.containsKey(readbackPV)) {
					final double basisValue = qPVMap.get(readbackPV)
							.doubleValue();
					record.updateReadback(basisValue, ch_noiseMap,
							ch_offsetMap, this);
				}
			} catch (Exception e) {
				System.err.println(e.getMessage());
			...
			try {
				...
				if (bookChannel != null) {
					if (bookChannel.connectAndWait()) {
						final double bookField = magnet.toCAFromField(magnet
								.getDfltField());
	...
	private void putSetPVs() {
		...
		for (final Electromagnet em : mags) {
			try {
				...
				// System.out.println("Ready to put " + setting + " to " +
				// ch.getId());
				ch.putValCallback(setting, this);

				if (em instanceof TrimmedQuadrupole) {
					Channel trimChannel = ((TrimmedQuadrupole) em)
							.getTrimSupply().getAndConnectChannel(
									MagnetTrimSupply.FIELD_SET_HANDLE);
					// System.out.println("Ready to put " + 0.0 + " to " +
					// trimChannel.getId());
		...
		for (final RfCavity rfCavity : rfCavities) {
			try {
				...
				// System.out.println("Ready to put " + rfCavity.getDfltCavAmp()
				// + " to " + ampSetCh.getId());
				if (rfCavity instanceof xal.smf.impl.SCLCavity) {
					ampSetCh.putValCallback(rfCavity.getDfltCavAmp()
							* ((SCLCavity) rfCavity).getStructureTTF(), this);
				} else {
					ampSetCh.putValCallback(rfCavity.getDfltCavAmp(), this);
				}
				Channel phaseSetCh = rfCavity
						.getAndConnectChannel(RfCavity.CAV_PHASE_SET_HANDLE);
				// System.out.println("Ready to put " +
				// rfCavity.getDfltCavPhase() + " to " + phaseSetCh.getId());
		...
		for (final Electromagnet em : mags) {
			try {
				Channel ch = em.getMainSupply().getAndConnectChannel(
						MagnetMainSupply.FIELD_SET_HANDLE);
				// System.out.println("Ready to put " +
				// Math.abs(em.getDfltField()) + " to " + ch.getId());

				final String channelID = ch.getId();
				if (qPSPVMap.containsKey(channelID))
					ch.putValCallback(qPSPVMap.get(channelID).doubleValue(),
							this);
			} catch (NoSuchChannelException e) {
				System.err.println(e.getMessage());
			} catch (ConnectionException e) {
	...
	protected void putDiagPVs() {
		...
		for (final BPM bpm : bpms) {
			...
			try {
				ProbeState probeState = model.getTrajectory().stateForElement(
						bpm.getId());
				// System.out.println("Now updating " + bpm.getId());

				if (probeState instanceof ICoordinateState) {
					final PhaseVector coordinates = ((ICoordinateState) probeState)
							.getFixedOrbit();
					// For SNS Ring BPM system, we only measure the signal with
					// respect to the center of the beam pipe.

					// TO-DO: the turn by turn arrays should really be generated
					// from betatron motion rather than random data about the
					// nominal
					final double[] xTBT = NoiseGenerator
							.noisyArrayForNominal(coordinates.getx() * 1000.0,
									DEFAULT_BPM_WAVEFORM_SIZE,
									DEFAULT_BPM_WAVEFORM_DATA_SIZE, bpmNoise,
									bpmOffset);
					final double xAvg = NoiseGenerator.getAverage(xTBT,
							DEFAULT_BPM_WAVEFORM_DATA_SIZE);

					final double[] yTBT = NoiseGenerator
							.noisyArrayForNominal(coordinates.gety() * 1000.0,
									DEFAULT_BPM_WAVEFORM_SIZE,
									DEFAULT_BPM_WAVEFORM_DATA_SIZE, bpmNoise,
									bpmOffset);
					final double yAvg = NoiseGenerator.getAverage(yTBT,
							DEFAULT_BPM_WAVEFORM_DATA_SIZE);

					bpmXAvgChannel.putValCallback(xAvg, this);
					// bpmXTBTChannel.putValCallback( xTBT, this ); // don't
					// post to channel access until the turn by turn data is
					// generated correctly
					bpmYAvgChannel.putValCallback(yAvg, this);
					// bpmYTBTChannel.putValCallback( yTBT, this ); // don't
					// post to channel access until the turn by turn data is
					// generated correctly
				...
				// hardwired BPM amplitude noise and offset to 5% and 0.1mm
				// (randomly) respectively
				bpmAmpAvgChannel.putVal(NoiseGenerator
						.setValForPV(20., 5., 0.1));
				// calculate the BPM phase (for linac only)
				if (!(myProbe instanceof TransferMapProbe)
						&& !(bpm instanceof RingBPM)) {
					final Channel bpmPhaseAvgChannel = bpm
							.getChannel(BPM.PHASE_AVG_HANDLE);
					bpmPhaseAvgChannel.putValCallback(
							probeState.getTime()
									* 360.
									* (((BPMBucket) bpm.getBucket("bpm"))
											.getFrequency() * 1.e6) % 360.0,
							this);
				}
			} catch (ConnectionException e) {
				System.err.println(e.getMessage());
		...
		for (final ProfileMonitor ws : wss) {
			...
			try {
				...
				if (model.getProbe() instanceof EnvelopeProbe) {
					final Twiss[] twiss = ((EnvelopeProbeState) probeState)
							.getCorrelationMatrix().computeTwiss();
					wsX.putValCallback(twiss[0].getEnvelopeRadius() * 1000.,
							this);
					wsY.putValCallback(twiss[1].getEnvelopeRadius() * 1000.,
							this);
					double WSposition = ws.getPosition();
					tempWSp.add(WSposition);
		...
		/**showWSPlot*/
		try {
			_diagplot.showWSPlot(wsp, wsx, wsy);
		} catch (ConnectionException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		} catch (GetException e1) {
		...
		for (final BPM bpm : bpms) {
			...
			try {
				...
				if (bpmXMap.containsKey(bpmX.getId())) {
					bpmX.putVal(NoiseGenerator.setValForPV(
							bpmXMap.get(bpmX.getId()).doubleValue(), bpmNoise,
							bpmOffset));
				}

				if (bpmYMap.containsKey(bpmY.getId())) {
					bpmY.putVal(NoiseGenerator.setValForPV(
							bpmYMap.get(bpmY.getId()).doubleValue(), bpmNoise,
							bpmOffset));
				}

				// BPM amplitude
				if (bpmAmpMap.containsKey(bpmAmp.getId()))
					bpmAmp.putVal(NoiseGenerator.setValForPV(
							bpmAmpMap.get(bpmAmp.getId()).doubleValue(), 5.,
							0.1));
				// BPM phase (for linac only)
				if (!(myProbe instanceof TransferMapProbe)) {
					Channel bpmPhase = bpm.getChannel(BPM.PHASE_AVG_HANDLE);
					if (bpmPhaseMap.containsKey(bpmPhase.getId())) {
						bpmPhase.putVal(bpmPhaseMap.get(bpmPhase.getId())
								.doubleValue());
					}
				}
	...
	private void configureReadbacks() {
		...
		if (selectedSequence != null) {
			// for magnet PVs
			for (final Electromagnet em : mags) {
				READBACK_SET_RECORDS.add(new ReadbackSetRecord(em, em
						.getChannel(Electromagnet.FIELD_RB_HANDLE), em
						.getChannel(MagnetMainSupply.FIELD_SET_HANDLE)));

				// System.out.println(em.getChannel(
				// Electromagnet.FIELD_RB_HANDLE ));

				// handle the trimmed magnets
				if (em.isKindOf(TrimmedQuadrupole.s_strType)) {
					READBACK_SET_RECORDS.add(new ReadbackSetRecord(em, em
							.getChannel(MagnetTrimSupply.FIELD_RB_HANDLE), em
							.getChannel(MagnetTrimSupply.FIELD_SET_HANDLE)));
					ch_noiseMap.put(
							em.getChannel(MagnetTrimSupply.FIELD_RB_HANDLE),
							0.0);
					ch_offsetMap.put(
							em.getChannel(MagnetTrimSupply.FIELD_RB_HANDLE),
							0.0);
				...
				// set up the map between the magnet readback PV and its noise
				// level
				if (em.isKindOf(Quadrupole.s_strType)) {
					...
					ch_offsetMap.put(
							em.getChannel(Electromagnet.FIELD_RB_HANDLE),
							dipoleOffset);
				} else if (em.isKindOf(HDipoleCorr.s_strType)
						|| em.isKindOf(VDipoleCorr.s_strType)) {
					ch_noiseMap.put(
							em.getChannel(Electromagnet.FIELD_RB_HANDLE),
							correctorNoise);
					ch_offsetMap.put(
			...
			for (final RfCavity rfCav : rfCavities) {
				READBACK_SET_RECORDS.add(new ReadbackSetRecord(rfCav, rfCav
						.getChannel(RfCavity.CAV_AMP_AVG_HANDLE), rfCav
						.getChannel(RfCavity.CAV_AMP_SET_HANDLE)));
				READBACK_SET_RECORDS.add(new ReadbackSetRecord(rfCav, rfCav
						.getChannel(RfCavity.CAV_PHASE_AVG_HANDLE), rfCav
						.getChannel(RfCavity.CAV_PHASE_SET_HANDLE)));
				ch_noiseMap.put(rfCav.getChannel(RfCavity.CAV_AMP_AVG_HANDLE),
						rfAmpNoise);
				ch_noiseMap.put(
						rfCav.getChannel(RfCavity.CAV_PHASE_AVG_HANDLE),
						rfPhaseNoise);
				ch_offsetMap.put(rfCav.getChannel(RfCavity.CAV_AMP_AVG_HANDLE),
						rfAmpOffset);
				ch_offsetMap.put(
						rfCav.getChannel(RfCavity.CAV_PHASE_AVG_HANDLE),
						rfPhaseOffset);
			...
			Collections.sort(READBACK_SET_RECORDS,
					new ReadbackSetRecordPositionComparator(selectedSequence));
			READBACK_SET_TABLE_MODEL
					.setRecords(new ArrayList<ReadbackSetRecord>(
							READBACK_SET_RECORDS));

			// add by liyong
			/*
			 * final int rowCount = READBACK_SET_TABLE_MODEL.getRowCount();
			 * for(int row=0;row<rowCount;row++){
			 * READBACK_SET_TABLE_MODEL.isCellEditable(row, 4); }
			 */
		}
	}
	...
	public void acceleratorChanged() {
		if (accelerator != null) {
			stopServer();
			_repRateChannel = accelerator.getTimingCenter().findChannel(
					TimingCenter.REP_RATE_HANDLE);
			beamOnEvent = accelerator.getTimingCenter().findChannel(
					TimingCenter.BEAM_ON_EVENT_HANDLE);
			beamOnEventCount = accelerator.getTimingCenter().findChannel(
					TimingCenter.BEAM_ON_EVENT_COUNT_HANDLE);
			slowDiagEvent = accelerator.getTimingCenter().findChannel(
					TimingCenter.SLOW_DIAGNOSTIC_EVENT_HANDLE);
			setHasChanges(true);
		}
	}
	...
	public void selectedSequenceChanged() {
		...
		if (selectedSequence != null) {
			...
			// get electro magnets
			TypeQualifier typeQualifier = QualifierFactory
					.qualifierWithStatusAndTypes(true, Electromagnet.s_strType);
			mags = getSelectedSequence()
					.<Electromagnet> getAllNodesWithQualifier(typeQualifier);

			// get all the rf cavities
			typeQualifier = typeQualifier = QualifierFactory
					.qualifierWithStatusAndTypes(true, RfCavity.s_strType);
			rfCavities = getSelectedSequence()
					.getAllInclusiveNodesWithQualifier(typeQualifier);

			// get all the BPMs
			bpms = getSelectedSequence().<BPM> getAllNodesWithQualifier(
					QualifierFactory.qualifierWithStatusAndType(true, "BPM"));

			// get all the wire scanners
			wss = getSelectedSequence().getAllNodesWithQualifier(
					QualifierFactory.qualifierWithStatusAndType(true,
							ProfileMonitor.PROFILE_MONITOR_TYPE));
...
class ReadbackSetRecordPositionComparator implements
		Comparator<ReadbackSetRecord> {
	 ...
	 * sequence
	 */
	public int compare(final ReadbackSetRecord record1,
			final ReadbackSetRecord record2) {
...
class DiagPlot {
	...
	protected FunctionGraphsJPanel _bpmplot;
	protected FunctionGraphsJPanel _wsplot;
	...
	public DiagPlot(FunctionGraphsJPanel bpmplot, FunctionGraphsJPanel wsplot) {
		_bpmplot=bpmplot;
		_wsplot=wsplot;
		setupPlot(bpmplot,wsplot);
	}

	public void showBPMPlot(double[] p,double[] x, double[] y) throws ConnectionException, GetException {
		_bpmplot.removeAllGraphData();
	    ...
	    DataBPMx.setGraphProperty(_bpmplot.getLegendKeyString(), "xAvg");
		DataBPMy.setGraphProperty(_bpmplot.getLegendKeyString(), "yAvg");	   
		_bpmplot.addGraphData(DataBPMx);
		_bpmplot.addGraphData(DataBPMy);	
		    
		
	}

	
	public void showWSPlot(double[] wsp,double[] wsx, double[] wsy) throws ConnectionException, GetException {
		_wsplot.removeAllGraphData();
	    ...
	    DataWSx.setGraphProperty(_wsplot.getLegendKeyString(), "sigmax");
		DataWSy.setGraphProperty(_wsplot.getLegendKeyString(), "sigmay");	   
		_wsplot.addGraphData(DataWSx);
		_wsplot.addGraphData(DataWSy);			    	
	}
	
	
	public void setupPlot(FunctionGraphsJPanel bpmplot,FunctionGraphsJPanel wsplot) {
		/** setup bpmplot*/
		// labels
		bpmplot.setName( "BPM_PLOT" );
		bpmplot.setAxisNameX("Position(m)");
		bpmplot.setAxisNameY("Beam displacement (mm)");

		bpmplot.setNumberFormatX( new DecimalFormat( "0.00E0" ) );
		bpmplot.setNumberFormatY( new DecimalFormat( "0.00E0" ) );
		
		// add the horizontal and vertical guides
		bpmplot.addVerticalLine( bpmplot.getCurrentMinX(), Color.black );
		bpmplot.addVerticalLine( bpmplot.getCurrentMaxX(), Color.black );
		bpmplot.addHorizontalLine( bpmplot.getCurrentMinY(), Color.black );
		bpmplot.addHorizontalLine( bpmplot.getCurrentMaxY(), Color.black );

		// Hide buttons on the plot since they are handled by a dialog box
		bpmplot.setChooseModeButtonVisible( false );
		bpmplot.setHorLinesButtonVisible( false );
		bpmplot.setVerLinesButtonVisible( false );

		// Show the horizontal and vertical guides
		bpmplot.setDraggingHorLinesGraphMode( true );
		bpmplot.setDraggingVerLinesGraphMode( true );

		// add legend support
		bpmplot.setLegendPosition( FunctionGraphsJPanel.LEGEND_POSITION_ARBITRARY );
		bpmplot.setLegendKeyString( "Legend" );
		bpmplot.setLegendBackground( Color.lightGray );
		bpmplot.setLegendColor( Color.black );
		bpmplot.setLegendVisible( true );		
		
		/** setup wsplot*/
		// labels
		wsplot.setName( "WS_PLOT" );
		wsplot.setAxisNameX("Position(m)");
		wsplot.setAxisNameY("Beam Envelope(mm)");

		wsplot.setNumberFormatX( new DecimalFormat( "0.00E0" ) );
		wsplot.setNumberFormatY( new DecimalFormat( "0.00E0" ) );
		
		// add the horizontal and vertical guides
		wsplot.addVerticalLine( wsplot.getCurrentMinX(), Color.black );
		wsplot.addVerticalLine( wsplot.getCurrentMaxX(), Color.black );
		wsplot.addHorizontalLine( wsplot.getCurrentMinY(), Color.black );
		wsplot.addHorizontalLine( wsplot.getCurrentMaxY(), Color.black );

		// Hide buttons on the plot since they are handled by a dialog box
		wsplot.setChooseModeButtonVisible( false );
		wsplot.setHorLinesButtonVisible( false );
		wsplot.setVerLinesButtonVisible( false );

		// Show the horizontal and vertical guides
		wsplot.setDraggingHorLinesGraphMode( true );
		wsplot.setDraggingVerLinesGraphMode( true );

		// add legend support
		wsplot.setLegendPosition( FunctionGraphsJPanel.LEGEND_POSITION_ARBITRARY );
		wsplot.setLegendKeyString( "Legend" );
		wsplot.setLegendBackground( Color.lightGray );
		wsplot.setLegendColor( Color.black );
		wsplot.setLegendVisible( true );
